SonarQube is an open-source platform developed by SonarSource for continuous inspection of code quality to perform automatic reviews with static analysis of code to detect bugs, code smells, and security vulnerabilities. It supports over 20 programming languages and can track issues in applications and provide reports, thereby helping teams to manage code quality over time.

# sonar manual installtion
...
inside the jenkins go to global tool configuration (plugins)

#company Requirement bases on select a version
...
```docker
	docker run --rm -v <Maven_Project_Path>:/usr/src -w /usr/src sonarsource/sonar-scanner-cli \
